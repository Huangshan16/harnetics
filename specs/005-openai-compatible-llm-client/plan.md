...
将远端 LLM 调用从 LiteLLM 的 provider/model 专属语义收敛到 OpenAI-compatible 原生会话接口：对 OpenAI-compatible 网关直接传递原始模型名和 `base_url`，草稿生成与影响分析复用同一调用规则；保留显式本地配置路径，并强化状态端点对 effective route 的可观测性。
...
**Storage**: SQLite (graph DB) + ChromaDB (existing vector store, unchanged for this feature)  
**Testing**: pytest  
...
**Constraints**: 不泄漏 API key；远端 OpenAI-compatible 网关与本地显式配置都必须可用；不扩大到 embedding 路由重构  
**Scale/Scope**: 覆盖草稿生成、影响分析 AI 判定与状态诊断 3 条后端 LLM 入口；前端仅复用现有状态展示能力
...
- 最小改动：PASS。当前特性聚焦 LLM 调用链与运行时诊断，不扩散到 embedding 主路径。
