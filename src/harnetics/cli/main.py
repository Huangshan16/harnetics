...
@app.command()
def init(
    db: str = typer.Option("var/harnetics.db", help="SQLite 数据库路径"),
) -> None:
    """初始化图谱数据库（建表 + WAL 模式）。"""
...
def ingest(
    ...
    db: str = typer.Option("var/harnetics.db", help="SQLite 数据库路径"),
    chroma: str = typer.Option("var/chroma", help="ChromaDB 持久化目录"),
) -> None:
    """将文档批量导入图谱数据库与向量索引。"""
    from harnetics.graph.store import init_db
    ...
    emb_store = EmbeddingStore(persist_dir=chroma)
    indexer = DocumentIndexer(db_path=db_path, embedding_store=emb_store)
...
def serve(
    ...
    os.environ.setdefault("HARNETICS_GRAPH_DB_PATH", db)
