...
import { documents, icdSections, graphEdges, type DocStatus } from '@/data/mock'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Separator } from '@/components/ui/separator'

const statusConfig: Record<DocStatus, { label: string; variant: 'success' | 'warning' | 'secondary' | 'destructive' }> = {
...
export default function DocumentDetail() {
  ...
  const upstream = graphEdges.filter((e) => e.source === doc.doc_id)
  const downstream = graphEdges.filter((e) => e.target === doc.doc_id)
  // ICD-001 (id="1") gets full sections; others get placeholder
  const sections = doc.id === '1' ? icdSections : []
    ...
    <div className="container mx-auto max-w-screen-xl px-4 py-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-sm">{doc.doc_id} · {doc.version} · 最近更新：{doc.last_updated}</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-4">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">上游文档（本文档引用）</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                {upstream.length === 0 ? (
                  ...
                  upstream.map((e, i) => {
                    const target = documents.find((d) => d.doc_id === e.target)
                    return (
                      <div key={i} className="flex items-center justify-between text-sm">
                        ...
                        <Badge variant="outline" className="text-xs">{relationLabel[e.relation]}</Badge>
                      </div>
                    )
                  })
                )}
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">下游文档（引用本文档）</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                {downstream.length === 0 ? (
                  ...
                  downstream.map((e, i) => {
                    const src = documents.find((d) => d.doc_id === e.source)
                    return (
                      <div key={i} className="flex items-center justify-between text-sm">
                        <Link
                          to={`/documents/${src?.id}`}
          ...
          <div className="space-y-2">
            <h2 className="text-base font-semibold flex items-center gap-2">
              ...
              文档章节
              <Badge variant="secondary" className="text-xs">{doc.section_count} 节</Badge>
            </h2>
            {sections.length === 0 ? (
              <Card>
