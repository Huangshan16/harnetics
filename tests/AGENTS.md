...
test_llm_client.py: LLM 配置契约测试，锁定 Ollama 裸模型名归一化与目标模型存在性判断。
...
test_graph_store.py: 图谱存储与索引回归测试，锁定 graph schema、section 入库与目录导入边界。
test_e2e_mvp_scenario.py: 图谱栈端到端场景测试，覆盖上传、草稿、评估、影响分析、图谱与仪表盘契约；新增草稿路由 LLM 设置透传与章节级 impact 定位断言。
